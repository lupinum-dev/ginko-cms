 ...
 *
 * Source: ginko-content/packages/content/src/core/content/path.ts
 *
...
export const isPartialPath = (path: string): boolean =>
  path.split(/[:/]/).some((part) => /^_.*/.test(part))
...
export const generatePath = (
  ...
  }: { forceLeadingSlash?: boolean; respectPathCase?: boolean } = {},
): string => {
...
export const generateCanonicalKey = (
  parts: string[],
  {
    ...
    respectPathCase = false,
  }: { translatedSlugs?: boolean; respectPathCase?: boolean } = {},
): string => {
  const segments = parts.map(parseSegment).filter((segment) => segment.refined.length > 0)

  if (!translatedSlugs) {
    return (
      generatePath(segments.map((segment) => segment.refined).join('/'), { respectPathCase }) || '/'
    )
  }
...
export const stripLocalePrefix = (
  ...
  const routeLocale =
    segments[0] && locales.includes(segments[0]) && segments[0] !== defaultLocale
      ? segments[0]
      : undefined
  const locale =
    explicitLocale && locales.includes(explicitLocale)
      ? explicitLocale
...
export const routeToContentPathCandidates = (
  ...
  const locales = Array.from(
    new Set(
      [...Object.keys(mounts || {}), ...localeChain, defaultLocale].filter(Boolean) as string[],
    ),
  )
  ...
  const matchedMount =
    activeMount && (stripped.path === activeMount || stripped.path.startsWith(`${activeMount}/`))
      ? activeMount
