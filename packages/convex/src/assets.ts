...
import { getCollection, normalizeCollectionDoc } from './lib/collections.js'
...
async function loadAssetRelationships(
  ...
  for (const row of referenceRows) {
    ...
    if (!current || locale === defaultLocale) entryById.set(entryId, entryMeta)
    target.push({
      entryId,
  ...
  for (const asset of assets) {
    ...
    const meta = resolvedMetadata.get(`${toStringId(asset.entryId)}:${defaultLocale}`)
    if (meta) entryById.set(toStringId(asset.entryId), meta)
  }
