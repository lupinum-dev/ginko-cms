...
export async function runInitCommand(cwd: string, io: CliIo): Promise<number> {
  ...
  if (checkResult === 0) {
    write(
      io.stdout,
      [
        `Next: run \`pnpm exec ginko-cms doctor\`, then \`pnpm exec convex dev --once --tail-logs disable --typecheck disable\`, then \`pnpm exec ginko-cms push\`.`,
        ...
        `Set \`CONVEX_IDENTITY_FORWARDING_KEY\` or \`GINKO_CMS_COMPONENT_FORWARDING_KEY\` in the Nuxt app/server environment.`,
