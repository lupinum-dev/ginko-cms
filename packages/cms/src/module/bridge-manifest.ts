...
import {
  ...
  type ComponentBridgeModule,
  type ComponentBridgeManifest,
} from '@lupinum/trellis-bridge/manifest'

import {
  bridgeModuleRegistry,
  defaultBridgeImports,
  type BridgeRegistryModule,
} from '../bridge/registry.js'

const generatedFiles = ['convex/auth.ts', 'convex/http.ts', 'convex/ginkoCms/_caller.ts'] as const

function ginkoModule(
  relativePath: string,
  factoryName: string,
  factoryPath: string,
...
function bridgeRegistryModule(module: BridgeRegistryModule): ComponentBridgeModule {
...
const bridgeModules: ComponentBridgeModule[] = bridgeModuleRegistry.map(bridgeRegistryModule)

function findPackageRoot(startFileUrl: string): string {
...
function removeDanglingCommas(source: string): string {
...
function isStandaloneUseForApp(line: string, appName: string, componentName: string): boolean {
  const normalized = normalizeLine(line)
  const call = `${appName}.use(${componentName})`
  return normalized === call || normalized === `${call};`
}

function hasBetterAuthRegistrationForApp(source: string, appName: string): boolean {
...
function includesImport(source: string, binding: string, specifier: string): boolean {
  const pattern = new RegExp(
...
function includesNamedImport(source: string, name: string, specifier: string): boolean {
  const pattern = new RegExp(
...
function requireExistingHostSetup(
  relativePath: string,
  current: string,
  checks: readonly { ok: boolean; message: string }[],
  snippet: string,
): string {
...
export const ginkoCmsBridgeManifest: ComponentBridgeManifest = defineComponentBridgeManifest({
