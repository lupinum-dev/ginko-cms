...
export default defineMcpPrompt({
  ...
  handler: async ({ collection, entryId, sourceLocale, targetLocale }) => {
    return [
      ...
      '5. Use `get-readiness-detail` as readiness context. If the credential has publish permission, use `preview-publish` with the active `agentRunId` for publish-impact context when route, SEO, sitemap, search, nav, or publish blockers matter. It does not save or publish content.',
