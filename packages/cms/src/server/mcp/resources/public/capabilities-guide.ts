    ...
    contents: [
      {
        ...
        text: [
          ...
          '',
          'Studio shows the same workflow an agent should follow: inspect collection capability, inspect entry locale state, run `explain-public-visibility`, call `preview-publish` as needed, then create a human review request with `request-publish-review` when publishing is ready.',
          '',
          ...
          '',
          'For translation prep, compare source and target locale data, check whether the target locale draft exists, inspect missing required fields and route blockers, and use `preview-publish` only as readiness context. Previewing never publishes content.',
          '',
