...
**Follow-up polish wave: implemented** (same day, W1–W10 on this branch) — everything
deferred above shipped (restore action, queue deep links, writing-surface editor,
state-code status tones + next-action button, full i18n of the periphery, dark elevation
ladder + motion tokens, asset-browser decomposition, write-time activity actor labels),
