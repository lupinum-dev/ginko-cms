...
describe('canonical draft lifecycle', () => {
  it('rejects stale autosave and never creates editorial history for autosaves', async () => {
    const ctx = createCtx()
    await seedOwner(ctx)
    ...
    ).resolves.toEqual({ draftVersion: 2, dirtyLocales: ['en'] })
  ...
  it('keeps immutable public output unchanged while shared and locale drafts diverge', async () => {
    const ctx = createCtx()
    await seedOwner(ctx)
    await seedSettings(ctx)
    const { entryId } = await seedEditorFixture(ctx)
