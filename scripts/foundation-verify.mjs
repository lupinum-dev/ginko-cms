...
const workRoot = resolve(repoRoot, '..')
const trellisRoot = resolve(workRoot, 'trellis')
...
function runWithRetry(label, cwd, command, args, options = {}) {
  const attempts = options.attempts ?? 2
  let lastError
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      run(attempt === 1 ? label : `${label} retry ${attempt}`, cwd, command, args, options)
      return
    } catch (error) {
      lastError = error
      if (attempt >= attempts) break
      console.warn(`${label} failed on attempt ${attempt}; retrying once.`)
    }
  }
  throw lastError
}
...
function removeLegacyManagedCommentsFromConsumer() {
  const convexConfigPath = resolve(consumerRoot, 'convex', 'convex.config.ts')
  if (!existsSync(convexConfigPath)) return

  const current = readFileSync(convexConfigPath, 'utf8')
  const next = current
    .split('\n')
    .filter(
      (line) =>
        !line.includes('@trellis-managed-start: @lupinum/ginko-cms convex-component') &&
        !line.includes('@trellis-managed-end: @lupinum/ginko-cms convex-component'),
    )
    .join('\n')

  if (next !== current) {
    console.log(
      `Removed obsolete Ginko managed-comment markers from ${convexConfigPath}; host setup remains explicit.`,
    )
    writeFileSync(convexConfigPath, next)
  }
}
...
function prepareConsumerPackageDependencies() {
  ...
  const packageTarballs = {
    ...
    '@lupinum/trellis': findPackedTarball('@lupinum/trellis'),
    '@lupinum/trellis-bridge': findPackedTarball('@lupinum/trellis-bridge'),
  }
  ...
  const next = current
    ...
    .filter(
      (line) =>
        !line.includes('../../0_libs/WORK/trellis') &&
...
function restoreWorkspaceInstalls() {
  const workspaces = [
    ['Restore Trellis install', trellisRoot],
    ['Restore Ginko install', repoRoot],
  ]
...
export function runFoundationVerify() {
  ...
  try {
    ...
    console.log('Do not run package:e2e or Trellis packing concurrently with consumer builds.')
    if (releaseMode) {
    ...
    run('Trellis install', trellisRoot, 'pnpm', ['install'])
    run('Trellis publish-surface validation', trellisRoot, 'pnpm', ['run', 'check:publish-surface'])
    runWithRetry('Trellis runtime build', trellisRoot, 'pnpm', ['run', 'build:module'])
    run('Trellis CLI build', trellisRoot, 'pnpm', ['run', 'build:cli'])
    run('Trellis bridge package build', trellisRoot, 'pnpm', [
      '--filter',
    ...
    run('Consumer install before setup', consumerRoot, 'pnpm', [
      'install',
      '--force',
      '--ignore-scripts',
    ])
    removeLegacyManagedCommentsFromConsumer()
    resetAutoGeneratedConsumerSetup()
    run('Consumer Ginko CMS setup', consumerRoot, 'pnpm', ['exec', 'ginko-cms', 'init'])
    run('Consumer Ginko CMS bridge check', consumerRoot, 'pnpm', [
      'exec',
      'ginko-cms',
      'bridge',
      'check',
